...
class CommitTest(utils.BareRepoTestCase):
    ...
    def test_new_commit_encoding(self):
        ...
        self.assertEqual(GIT_OBJ_COMMIT, commit.type)
        self.assertEqual(u'iso-8859-1', commit.message_encoding)
        self.assertEqual(message, commit.message)
        self.assertEqual(12346, commit.commit_time)
        self.assertEqualSignature(committer, commit.committer)
        self.assertEqualSignature(author, commit.author)
    ...
    def test_modify_commit(self):
        ...
        commit = self.repo[COMMIT_SHA]
        self.assertRaises(AttributeError, setattr, commit, u'message', message)
        self.assertRaises(AttributeError, setattr, commit, u'committer',
                          committer)
        self.assertRaises(AttributeError, setattr, commit, u'author', author)
        self.assertRaises(AttributeError, setattr, commit, u'tree', None)
        self.assertRaises(AttributeError, setattr, commit, u'parents', None)
